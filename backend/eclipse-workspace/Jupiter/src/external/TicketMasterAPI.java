...
public class TicketMasterAPI {
	...
	/**
	 * Take reference of Ticket Master API -- Event Search category
	 */

	// Convert JSONArray from TicketMaster API to a list of item objects.
		...
		if (!event.isNull(EMBEDDED)) {
			...
			if (!embedded.isNull(VENUES)) {
				...
				for (int i = 0; i < venues.length(); ++i) {
					...
					StringBuilder stringBuilder = new StringBuilder(); // address in single String required
					...
					if (!venue.isNull(COUNTRY)) {
						...
						if (!country.isNull(NAME)) {
							stringBuilder.append('\n');
							stringBuilder.append(country.getString(NAME));
						}
					}

					String addr = stringBuilder.toString();
					if (!addr.equals("")) { // check if two objects are identical, in general checking length is faster
						return addr;
					}
				}
			}
